...
def SN_integral(bispec,var_len, var_gal, var_xx, Ls, ls, Lls, ang, bin_size, sample1d, min_index, max_index):
    ...
    for i in np.arange(min_index,max_index+1):
        ...
        for j in np.arange(i,max_index+1): #start at l=L
            ...
            if len(Ll)>=1:    
                num   = (spec_int*2)**2
                denom = fac*splev(L_,var_lens,ext=0)*splev(l_const,var_lens,ext=0)*splev(Ll,var_gal,ext=0)
...
if __name__ == "__main__":  				
    ...
    plt.xlabel(r'$L_{min}$')
    plt.ylabel("S/N")
    plt.savefig("S_over_N_Toshiya_B_kkg_lmax%d_thetaFWHMarcmin10_noiseUkArcmin10_fsky%.1f.pdf"%(maxL_,fsky), bbox_inches="tight")
    plt.show()
    ...
    plt.xlabel(r'$L_{max}$')
    plt.ylabel("S/N")
    plt.savefig("S_over_N_Toshiya_B_kkg_lmin%d_thetaFWHMarcmin10_noiseUkArcmin10_fsky%.1f.pdf"%(minL_,fsky), bbox_inches="tight")
    plt.show()
