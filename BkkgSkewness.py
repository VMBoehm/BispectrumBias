    ...
    * R :smoothing scale
    ...
    for i in np.arange(0,len_L):
        ...
        for j in np.arange(0,len_l):
            ...
            Ll_const = Ll_[j*len_ang:(j+1)*len_ang]
            ...
            integrand=spec_int*l_const*np.exp(-0.5*(l_const**2+Ll_const**2+L_const**2)/R**2)
            
            phi_integral+=[simps(integrand, ang_int)]
     
        result+=[(simps(phi_integral,np.unique(l_)))]
       
    result=simps(result*L_f,L_f)
    return np.asarray(result)/((2*np.pi)**3)