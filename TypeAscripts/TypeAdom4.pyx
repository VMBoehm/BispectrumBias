...
cimport numpy as np
import time
#from Tools cimport sin, cos, sqrt
...
from Tools import simps#, interp1d_gsl
...
def get_typeA(double[:]  Ls, int sample1d, cl_fid, field , nl, int lmin, int lmax, double min_mu, double max_mu, int num):
	"""
	...
	* num: L index
	...
	cdef double l1,l,l2,l12,l1l,w,l1cos,l1sin,term1
	...
	for i in xrange(pP):
		...
		for j in xrange(N):
			...
			for k in xrange(K):
				...
				integrand_mu=l1l*w*cl_*b*(l1l*cos_diff-l2)*(l1l*cos_diff-l2-term1+L*l*cos_mu)
				integrand_l[k]=simps(integrand_mu,mus)*2.#factor of two because we are only integrating over half of the angle
...
ang_i, Ls ,l3s, splines = pickle.load(open(bispec_file))
...
assert(sample1d==len(l3s))
assert(np.allclose(Ls_,Ls))
